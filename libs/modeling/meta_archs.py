...
class TriDet(nn.Module):
    ...
    def __init__(
        ...
        self.upscale = Upscale(fpn_levels=len(self.fpn_strides), feat_dim=fpn_dim)
        self.scaletime_blocks = nn.Sequential(*[ScaleTime(channels=fpn_dim) for _ in range(3)])
    ...
    def forward(self, video_list, queries=None):
        ...
        for lvl, src in enumerate(srcs):
            ...
            lvl_pos_1d_embed = this_pos_1d_embeds + self.level_embed.weight[lvl].view(1, 1, -1)
        ...
        memory = self.deformable_encoder(src_flatten,
                                         spatial_shapes_1d,
                                         level_start_index_1d,
                                         pos=lvl_pos_1d_embed_flatten)
